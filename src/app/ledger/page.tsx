...
export default function ThreatLedger() {
  ...
  return (
    <div className="flex-grow bg-slate-50 pt-36 md:pt-48 pb-24 px-4 sm:px-6 min-h-screen">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-4">Community Threat Ledger</h1>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          {loading ? (
            <div className="p-16 text-center text-slate-500 font-medium">Connecting to live feed...</div>
          ) : threats.length === 0 ? (
            <div className="p-16 text-center text-slate-500 font-medium">No threats logged yet. The community is safe!</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {threats.map((threat) => (
                    <tr key={threat.id} className="hover:bg-blue-50/50 transition duration-150">
                      <td className="p-4 sm:p-5 text-sm text-slate-500 whitespace-nowrap">
                      ...
                      <td className="p-4 sm:p-5 text-sm">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-semibold text-xs uppercase tracking-wide
                          ${threat.type === 'url' ? 'bg-emerald-100 text-emerald-800' : 
                            threat.type === 'text' ? 'bg-indigo-100 text-indigo-800' : 
                            threat.type === 'image' ? 'bg-blue-100 text-blue-800' : 
                            'bg-amber-100 text-amber-800'}`}
                        >
                          {threat.type === 'url' ? '🌐 Link' : threat.type === 'text' ? '💬 SMS' : threat.type === 'image' ? '👁️ Image' : '🎙️ Audio'}
                        </span>
                      </td>
                      <td className="p-4 sm:p-5 text-sm text-slate-700 font-mono truncate max-w-[150px] sm:max-w-xs md:max-w-md">
                        {threat.content}
                      </td>
                      <td className="p-4 sm:p-5 text-sm text-red-700 font-medium max-w-[150px] sm:max-w-xs">
